...
6. For the C++ and CUDA implementations, we have changed to using separable filters for faster performance.
   This has given rise to small differences compared to previous versions. For our tests, we have therefore updated the `images/correct_*_{cpp|cuda}.{png|exr}` images.
